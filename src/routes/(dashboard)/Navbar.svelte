<script lang="ts">
  import { AppsMenu, UserMenu, NotificationList } from "$lib/components";
  import {
    DarkMode,
    Dropdown,
    DropdownItem,
  ...
  import type { NotificationProps } from "$lib/components/types";
  import { goto } from "$app/navigation";

  function handleSignOut() {
    goto('/authentication/logout', {invalidateAll: true});
  }
...
<Navbar class="mx-10 sm:mx-0 ">
  ...
  <div
    class="ms-auto flex items-center text-gray-500 sm:order-2 dark:text-gray-300"
  >
    <!-- <NotificationList {notifications} /> -->
    <!-- <AppsMenu {menu} /> -->
    <DarkMode
      size="lg"
      class="m-0.5 hover:text-gray-900 dark:hover:text-white"
    />
    <UserMenu {...Users[4]} {menuItems}>
      <DropdownDivider />
      <DropdownItem class="w-full text-left" onclick={handleSignOut}>Sign out</DropdownItem>
    </UserMenu>
  </div>
</Navbar>
